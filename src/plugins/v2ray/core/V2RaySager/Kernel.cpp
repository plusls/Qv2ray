...
V2RaySagerKernel::V2RaySagerKernel()
{
    ...
    connect(apiWorker, &APIWorker::OnAPIDataReady, this, &V2RaySagerKernel::OnStatsAvailable);
    kernelStarted = false;
}
...
void V2RaySagerKernel::SetProfileContent(const ProfileContent &content)
{
    profile = content;
}

bool V2RaySagerKernel::PrepareConfigurations()
{
    const auto config = V2RaySagerProfileGenerator::GenerateConfiguration(profile);
...
std::optional<QString> V2RaySagerKernel::ValidateConfig(const QString &path)
{
    const auto settings = V2RayCorePluginClass::PluginInstance->settings;
